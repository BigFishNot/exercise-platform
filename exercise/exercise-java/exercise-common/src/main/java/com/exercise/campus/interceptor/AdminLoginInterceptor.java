...
/**
 * 管理端登录拦截器
 * 读 header adminToken，查 Redis 登录组件，校验 roleType=ADMIN，写入登录上下文持有器
 */
@Component
public class AdminLoginInterceptor implements HandlerInterceptor {

    public static final String HEADER_TOKEN = "adminToken";
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        LoginRedisComponent.CacheValue v = loginRedisComponent.getAdmin(token);
        if (v == null) {
            throw new BusinessException(ResponseCodeEnum.UNAUTHORIZED);
        }
        if (!RoleTypeEnum.ADMIN.getRoleType().equals(v.getRoleType())) {
